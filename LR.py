...
class LinearRegression:
    ...
    def fit_and_predict(self, sensor_names, training_samples, training_RULs, configuration_samples, configuration_RULs,
        ...
        regr.fit(X_train, training_RULs)

        # Make predictions using the testing set
        predicted_RULs = regr.predict(X_test)
...
class CMAPSSData:
    ...
    def calculate_trendability(self):
        ...
        for feature_name in self.sensor_names:
            ...
            self.trendability_sensors[feature_name] = np.nan_to_num(trendability_feature)
...
class ShapExplainerModel:
    ...
    def predict_LR(self, qc):
        global model
        return np.array(model.regr.predict(qc))
    ...
    def calculate_trendability(self):
        ...
        for i in range(len(self.sensor_names)):
            ...
            if trendability_shap > 1:
                self.trendability_shap_values[self.sensor_names[i]] = 0
            else:
