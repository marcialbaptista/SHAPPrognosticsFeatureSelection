...
class MultiLayerPerceptron:
    ...
    def fit_and_predict(self, sensor_names, training_samples, training_RULs, configuration_samples, configuration_RULs,
                        validation_samples, validation_RULs, testing_samples, testing_RULs):
        ...
        nnet = neural_network.MLPRegressor(hidden_layer_sizes=(100,))

        # Train the model using the training sets
        nnet.fit(X_train, training_RULs)

        predicted_RULs = nnet.predict(X_test)
    ...
    def calculate_errors(self):
        ...
        fp = np.count_nonzero(
            np.array(self.predicted_RULs)[np.array(self.predicted_RULs) > np.array(self.testing_RULs) + 13])
        fn = np.count_nonzero(
        ...
        print("MAPE Error:", np.mean(np.abs((np.array(self.testing_RULs)[testing_RULs_non_zero] -
                                             np.array(self.predicted_RULs)[testing_RULs_non_zero]) /
                                            np.array(self.testing_RULs)[testing_RULs_non_zero])))
        print("RMSE Error:",
              math.sqrt(np.mean(np.power(np.abs(np.array(self.predicted_RULs) - np.array(self.testing_RULs)), 2))))
...
class CMAPSSData:
    ...
    def calculate_monotonicity(self, nr_points_slope = 10):
        ...
        for feature_name in self.sensor_names:
            ...
            for unit in self.testing_units:
                ...
                for index in range(nr_points_slope, life_unit, 1):
                    monotonicity_unit += math.copysign(1, smooth_signal[index] - smooth_signal[index - nr_points_slope]) / (life_unit - nr_points_slope)
    ...
    def calculate_trendability(self):
        ...
        for feature_name in self.sensor_names:
            ...
            print("Trendability", feature_name, self.monotonicity_sensors[feature_name])
...
class ShapExplainerModel:
    ...
    def predict_LR(self, qc):
        global model
        return np.array(model.nnet.predict(qc))

    def run(self, X_train, nsamples=1000):
        X_train_summary = shap.kmeans(X_train, 10)
        self.sensor_names = X_train.columns
        self.explainer = shap.KernelExplainer(self.predict_LR, X_train_summary, l1_reg="auto")
    ...
    def calculate_prognosability(self):
        ...
        for feature_name in self.sensor_names:
            ...
            self.prognosability_shap_values[feature_name] = np.nan_to_num(math.exp(- np.std(failure_values) / np.mean(bottom_values)))
            print("Prognosability shap values", feature_name, self.prognosability_shap_values[feature_name])
            i += 1
        ...
        for i in range(len(self.sensor_names)):
            ...
            for unit1 in unique_testing_units:
                ...
                for unit2 in unique_testing_units:
                    ...
                    if len(shap_values_unit2) <= 2 or len(shap_values_unit1) <= 2:
                        continue
            ...
            self.trendability_shap_values[self.sensor_names[i]] = np.nan_to_num(trendability_shap)
